...
class DatabaseService {
  ...
  Future<Database> _initDatabase() async {
    ...
    return await openDatabase(
      path,
      version: 3, // Increased version to add currencies table
      onCreate: _onCreate,
  ...
  Future<void> _insertDefaultData(Database db) async {
    ...
    final defaultTypes = [
      'الأسرة', 'الأب', 'الأم', 'الولد الأول', 'الولد الثاني',
      'السيارة', 'البناء', 'الجد', 'الجدة', 'متنوع'
    ];
    ...
    final defaultPaymentEntities = [
      'سوبر ماركت', 'مطعم', 'ملابس', 'مواصلات', 'فواتير',
      'صيدلية', 'مستشفى', 'تعليم', 'ترفيه', 'آخر'
    ];
    ...
    final defaultReceiptEntities = [
      'راتب شهري', 'سلفة على الراتب', 'أجر عمل', 'هدية', 'عيدية', 'آخر'
    ];
  ...
  Future<void> _insertDefaultCurrencies(Database db) async {
    ...
    final defaultCurrencies = [
      {'code': 'SAR', 'name_ar': 'ريال سعودي', 'name_en': 'Saudi Riyal', 'exchange_rate_to_usd': 0.27, 'is_default': 1},
      {'code': 'USD', 'name_ar': 'دولار أمريكي', 'name_en': 'US Dollar', 'exchange_rate_to_usd': 1.0, 'is_default': 0},
      {'code': 'EUR', 'name_ar': 'يورو', 'name_en': 'Euro', 'exchange_rate_to_usd': 1.1, 'is_default': 0},
      {'code': 'AED', 'name_ar': 'درهم إماراتي', 'name_en': 'UAE Dirham', 'exchange_rate_to_usd': 0.27, 'is_default': 0},
      {'code': 'OMR', 'name_ar': 'ريال عماني', 'name_en': 'Omani Rial', 'exchange_rate_to_usd': 2.6, 'is_default': 0},
      {'code': 'KWD', 'name_ar': 'دينار كويتي', 'name_en': 'Kuwaiti Dinar', 'exchange_rate_to_usd': 3.3, 'is_default': 0},
      {'code': 'SYP', 'name_ar': 'ليرة سورية', 'name_en': 'Syrian Pound', 'exchange_rate_to_usd': 0.0004, 'is_default': 0},
    ];
  ...
  Future<int> insertTransaction(model.Transaction transaction) async {
    ...
    print('إدراج معاملة: ${transaction.direction} - ${transaction.entity} - ${transaction.amount}');
  ...
  Future<List<model.Transaction>> getRecentTransactions({int limit = 5}) async {
    final db = await database;
    print('استعلام المعاملات الأخيرة - حد: $limit');
    final List<Map<String, dynamic>> maps = await db.query(
  }
  ...
  Future<Map<String, dynamic>> safeDeleteEntity(int id) async {
    ...
    // Delete the entity
    final deletedRows = await db.delete('entities', where: 'id = ?', whereArgs: [id]);
  ...
  Future<Map<String, dynamic>> safeDeleteExpenseType(int id) async {
    ...
    // Delete the type
    final deletedRows = await db.delete('expense_types', where: 'id = ?', whereArgs: [id]);
  ...
  Future<Map<String, dynamic>> safeDeletePaymentMethod(String name) async {
    ...
    final deletedRows = await db.delete('payment_methods', where: 'name = ?', whereArgs: [name]);
  ...
  Future<int> deletePaymentMethod(String name) async {
    final db = await database;
    return await db.delete('payment_methods', where: 'name = ?', whereArgs: [name]);
  }
  ...
  Future<String> exportToExcel() async {
    ...
    final fileName = 'exoda_transactions_${DateFormat('yyyyMMdd_HHmmss').format(DateTime.now())}.xlsx';
  ...
  Future<String> exportDatabaseFile() async {
    final tempDir = await getTemporaryDirectory();
    final fileName = 'exoda_backup_${DateFormat('yyyyMMdd_HHmmss').format(DateTime.now())}.db';
    final filePath = '${tempDir.path}/$fileName';
  ...
  Future<Map<String, dynamic>> getDatabaseDiagnostics() async {
    ...
    try {
      ...
      try {
        final transactionsSchema = await db.rawQuery("PRAGMA table_info(transactions)");
      ...
      for (String table in ['transactions', 'entities', 'expense_types', 'payment_methods', 'settings']) {
        try {
      ...
      // Recent transactions
      try {
        final recentTransactions = await db.query('transactions', orderBy: 'id DESC', limit: 3);
  ...
  Future<String> exportTransactionsToExcel() async {
    ...
    sheet.cell(CellIndex.indexByString('F1')).value = TextCellValue('طريقة الدفع');
    sheet.cell(CellIndex.indexByString('G1')).value = TextCellValue('الملاحظات');
    ...
    for (int i = 0; i < transactions.length; i++) {
      ...
      sheet.cell(CellIndex.indexByString('A$row')).value = IntCellValue(tx['id'] as int);
      sheet.cell(CellIndex.indexByString('B$row')).value = TextCellValue(tx['date'].toString());
      sheet.cell(CellIndex.indexByString('C$row')).value = TextCellValue(tx['direction'].toString());
      sheet.cell(CellIndex.indexByString('D$row')).value = TextCellValue(tx['entity'].toString());
      sheet.cell(CellIndex.indexByString('E$row')).value = DoubleCellValue(tx['amount'] as double);
      sheet.cell(CellIndex.indexByString('F$row')).value = TextCellValue(tx['paymentMethod'].toString());
      sheet.cell(CellIndex.indexByString('G$row')).value = TextCellValue((tx['notes'] ?? '').toString());
    }
  ...
  Future<Map<String, dynamic>> importFromExcelPreview(String filePath) async {
    try {
      ...
      int idxPaymentMethod = headerRow.indexWhere((h) => h == 'طريقة الدفع' || h.toLowerCase().contains('payment'));
      int idxAmount = headerRow.indexWhere((h) => h == 'المبلغ' || h.toLowerCase().contains('amount'));
      int idxPayEntity = headerRow.indexWhere((h) => h == 'جهة مدفوعات' || h.toLowerCase().contains('pay'));
      int idxReceiveEntity = headerRow.indexWhere((h) => h == 'جهة مقبوضات' || h.toLowerCase().contains('receive'));
      int idxDate = headerRow.indexWhere((h) => h == 'date' || h == 'التاريخ');
      if (idxAmount == -1 || idxDate == -1 || (idxPayEntity == -1 && idxReceiveEntity == -1)) return {'transactions': [], 'unknownEntities': []};
      ...
      for (int i = 1; i < sheet.maxRows; i++) {
        ...
        final amountStr = idxAmount >= 0 && idxAmount < row.length ? row[idxAmount]?.value : null;
        final paymentMethod = idxPaymentMethod >= 0 && idxPaymentMethod < row.length ? row[idxPaymentMethod]?.value?.toString() ?? '' : '';
        final payEntity = idxPayEntity >= 0 && idxPayEntity < row.length ? row[idxPayEntity]?.value?.toString() ?? '' : '';
        final receiveEntity = idxReceiveEntity >= 0 && idxReceiveEntity < row.length ? row[idxReceiveEntity]?.value?.toString() ?? '' : '';
        final dateStr = idxDate >= 0 && idxDate < row.length ? row[idxDate]?.value?.toString() ?? '' : '';
